...
struct form *is* bound (`SF::System::InputStream`). Types that SFML 3 models as dedicated C++ classes but
CSFML models as plain C scalars need no class of their own here: `Angle` is a `Float` (degrees),
`StencilValue` is an `Integer`, `RenderStates` is `SF::Graphics::RenderState`, and the vector types are
...
- [x] **Clock** — `SF::System::Clock` (`ext/system/clock.c`); `elapsed_time`, `restart!`, `reset!`,
      `start!`, `stop!`, `running?`, `copy`
- [x] **Vector2** — `SF::System::Vector2` (`ext/system/vec2.c`); setters still accept `[x, y]`
- [x] **Vector3** — `SF::System::Vector3` (`ext/system/vec3.c`)
- [x] **Time** — `SF::System::Time` (`ext/system/time.c`); arithmetic, comparison, unit conversions
- [x] **Sleep** — `SF.sleep` (`ext/system/sleep.c`)
- [x] **InputStream** — `SF::System::InputStream` (`ext/system/input_stream.c`); wraps any `#read`-able
      object and is accepted by every `from_stream` constructor
- [x] **Buffer** — `SF::System::Buffer` (`ext/system/buffer.c`); returned by `Image#save_to_memory`

## Window

OpenGL-based windows, events, input handling.

- [x] **WindowBase** — `SF::Window::WindowBase` (`ext/window/window_base.c`); wraps `sfWindowBase`, an OS
...
- [x] **Window** — `SF::Window::Window` (`ext/window/window.c`); wraps `sfRenderWindow` and derives from
...
- [x] **VideoMode** — `SF::Window::VideoMode` (`ext/window/video_mode.c`); includes `desktop_mode` and
      `fullscreen_modes`
- [x] **Event** — `SF::Window::Event` (`ext/window/event.c`, `ext/window/event_name.c`); every payload is
      exposed — text, key, mouse move/raw/button/wheel, joystick move/button/connect, touch, sensor
- [x] **Keyboard** — `SF::Window::Keyboard` (`ext/window/keyboard.c`); real-time `pressed?`,
      `scancode_pressed?`, `localize`, `delocalize`, `description`, virtual-keyboard toggle
- [x] **Mouse** — `SF::Window::Mouse` (`ext/window/mouse.c`)
- [x] **Joystick** — `SF::Window::Joystick` (`ext/window/joystick.c`); includes `identification`
- [x] **Touch** — `SF::Window::Touch` (`ext/window/touch.c`)
- [x] **Sensor** — `SF::Window::Sensor` (`ext/window/sensor.c`)
- [x] **Clipboard** — `SF::Window::Clipboard` (`ext/window/clipboard.c`)
- [x] **Cursor** — `SF::Window::Cursor` (`ext/window/cursor.c`)
- [x] **Context** / **ContextSettings** — `SF::Window::Context`, `SF::Window::ContextSettings`
      (`ext/window/context.c`, `ext/window/context_settings.c`)
- [x] **Vulkan** — `SF::Window::Vulkan` (`ext/window/vulkan.c`)

## Graphics

2D rendering: shapes, sprites, text, render targets.

- [x] **Transform** — `SF::Graphics::Transform` (`ext/graphics/transform.c`); a class wrapping
...
- [x] **Transformable** — `SF::Graphics::Transformable` (`ext/graphics/transformable.c`); a **module**
      ...
      the standalone object is `SF::Graphics::Transformable::Instance` (returned by `Transformable.new`;
      internal, not part of the public surface)
- [x] **Drawable** — `SF::Graphics::Drawable` mixin (`ext/graphics/drawable.c`)
- [x] **RenderStates** — `SF::Graphics::RenderState` (`ext/graphics/render_state.c`); blend mode, stencil
      mode, coordinate type, texture, shader and transform are all settable
- [x] **RenderTarget** — `SF::Graphics::RenderTarget` module (`ext/graphics/target.c`), included by
      ...
      `Window` and `RenderTexture` from `ext/graphics/render_target.inc`. `SF::Graphics::Target` remains
      as the legacy runtime-dispatch wrapper for `Drawable#draw`.
- [x] **RenderWindow** — `SF::Graphics::RenderWindow` (`ext/graphics/render_window.c`); derives from
      `Window` and includes `RenderTarget`. Creation, events and the window surface come from
      `Window` / `WindowBase`.
- [x] **RenderTexture** — `SF::Graphics::RenderTexture` (`ext/graphics/render_texture.c`)
- [x] **View** — `SF::Graphics::View` (`ext/graphics/view.c`); including `View.from_rect` and
      `scissor`/`scissor=`
- [x] **CircleShape** — `SF::Graphics::CircleShape` (`ext/graphics/circle.c`), also available as the
      `SF::Graphics::Circle` alias; derives from `Shape`
- [x] **RectangleShape** — `SF::Graphics::RectangleShape` (`ext/graphics/rectangle.c`); derives from `Shape`
- [x] **ConvexShape** — `SF::Graphics::ConvexShape` (`ext/graphics/polygon.c`); derives from `Shape`
- [x] **Shape** — `SF::Graphics::Shape` (`ext/graphics/shape.c`); the base of the built-in shapes, mixing in
      `Transformable` and `Drawable`. Subclass it and define `point_count`/`point` for a custom shape
- [x] **Sprite** — `SF::Graphics::Sprite` (`ext/graphics/sprite.c`); mixes in `Transformable` and `Drawable`
- [x] **Texture** — `SF::Graphics::Texture` (`ext/graphics/texture.c`); every constructor in both linear
      and sRGB form, plus `resize`/`resize_srgb` and `swap`
- [x] **Image** — `SF::Graphics::Image` (`ext/graphics/image.c`)
- [x] **Font** — `SF::Graphics::Font` (`ext/graphics/font.c`); `info` returns the family name
- [x] **Text** — `SF::Graphics::Text` (`ext/graphics/text.c`); mixes in `Transformable` and `Drawable`;
      `#string` goes through the UTF-32 entry
      points, so non-ASCII round-trips exactly
- [x] **Glyph** — `SF::Graphics::Glyph` (`ext/graphics/glyph.c`)
- [x] **Shader** — `SF::Graphics::Shader` (`ext/graphics/shader.c`); scalar/vector/color/int/bool/matrix
      uniforms, all six array uniforms (float, vec2-4, mat3, mat4), `set_current_texture`, plus a
      generic `uniform=`
- [x] **Color** — `SF::Graphics::Color` (`ext/graphics/color.c`)
- [x] **Rect** — `SF::Graphics::Rect` (`ext/graphics/rect.c`); `sfFloatRect` and `sfIntRect`
- [x] **BlendMode** — `SF::Graphics::BlendMode` (`ext/graphics/blend_mode.c`)
- [x] **StencilMode** — `SF::Graphics::StencilMode` (`ext/graphics/stencil_mode.c`)
...
- [x] **Listener** — `SF::Audio::Listener` (`ext/audio/listener.c`); global volume plus
      position/direction/velocity/up-vector/cone
- [x] **ListenerCone** / **SoundSourceCone** — `SF::Audio::SoundSourceCone` (`ext/audio/sound_source_cone.c`)
- [x] **SoundSource** — `SF::Audio::SoundSource` (`ext/audio/sound_source.c`), the base class of
...
- [x] **SoundStatus** / **SoundChannel** — `SF::Audio::SoundStatus`, `SF::Audio::SoundChannel`
      (`ext/audio/audio_enums.c`)
- [x] **SoundBuffer** — `SF::Audio::SoundBuffer` (`ext/audio/sound_buffer.c`); loading from file, memory,
      stream and raw samples, saving, sample access and channel map
- [x] **Sound** — `SF::Audio::Sound` (`ext/audio/sound.c`); derives from `SoundSource`
- [x] **SoundStream** — `SF::Audio::SoundStream` (`ext/audio/sound_stream.c`); derives from `SoundSource`,
      subclass and implement `#on_get_data` (and optionally `#on_seek`)
- [x] **SoundRecorder** — `SF::Audio::SoundRecorder` (`ext/audio/sound_recorder.c`); subclass and
      implement `#on_process` (and optionally `#on_start`/`#on_stop`)
- [x] **SoundBufferRecorder** — `SF::Audio::SoundBufferRecorder`
      (`ext/audio/sound_buffer_recorder.c`); derives from `SoundRecorder`
- [x] **Music** — `SF::Audio::Music` (`ext/audio/music.c`); derives from `SoundStream`; file, memory and
      stream sources, loop points
- [x] **EffectProcessor** — `SF::Audio::SoundSource#effect_processor=`; a Ruby proc is dispatched through
...
- [x] **IpAddress** — `SF::Network::IpAddress` (`ext/network/ip_address.c`); string/bytes/integer
      constructors, `NONE`/`ANY`/`LOCAL_HOST`/`BROADCAST`, local and public address lookup
- [x] **Packet** — `SF::Network::Packet` (`ext/network/packet.c`); raw data plus every typed reader/writer
      (booleans, all integer widths, floats, string)
- [x] **SocketSelector** — `SF::Network::SocketSelector` (`ext/network/socket_selector.c`); add/remove for
      each socket type, wait and readiness checks
- [x] **TcpSocket** / **TcpListener** — `SF::Network::TcpSocket`, `SF::Network::TcpListener`
      (`ext/network/tcp_socket.c`, `ext/network/tcp_listener.c`); connect/accept, blocking control,
      raw and packet sends/receives
- [x] **UdpSocket** — `SF::Network::UdpSocket` (`ext/network/udp_socket.c`); bind/send/receive and packet
      variants, datagram size and any-port helpers
- [x] **Http** — `SF::Network::Http`, `SF::Network::HttpRequest`, `SF::Network::HttpResponse` (`ext/network/http.c`)
- [x] **Ftp** — `SF::Network::Ftp`, `SF::Network::FtpResponse`, `SF::Network::FtpDirectoryResponse`,
      `SF::Network::FtpListingResponse` (`ext/network/ftp.c`)
...
- **Needs a new CSFML binding** — new C entry points the gem would wrap in new `.c` files:
  - **Dns** — new `SF::Network::Dns` module mirroring `sf::Dns`: `resolve`, `query_ns`, `query_mx`,
    `query_srv`, `query_txt`, `get_public_address`, plus `SF::Network::Dns::MxRecord` /
    `SF::Network::Dns::SrvRecord`.
  - **Sftp** — new `SF::Network::Sftp` client (and its `Result`/`PathResult`/`Attributes`/
    `AttributesResult`/`ListingResult`/`SessionInfo`/`HostKey` types). Upstream positions SFTP as
    the replacement for FTP.
  - **PlaybackDevice** — a `SF::Audio::PlaybackDevice` module for `sf::PlaybackDevice`, notably
    `get_device_sample_rate`.
  - **version()** — runtime `SF.version` / library-version constant backed by `sf::version()`
    (today `SF::VERSION` is only the gem's own version).
...
the `sf::Style` namespace (already handled as window-style symbols in `SF::Window::Window`'s constructor).
...
| Symbols | Why |
| --- | --- |
| `sfWindow_*` (~26 functions) | A plain `sfWindow` (an OpenGL context with no render-target surface); `SF::Window::Window` wraps `sfRenderWindow` and `SF::Window::WindowBase` wraps `sfWindowBase`. |
...
| `sfShape_getPoint`, `sfShape_getPointCount` | `SF::Graphics::Shape` reads these from the Ruby subclass, which is where they are defined. |
| `sfTexture_updateFromWindow` | `Texture#update_from_window` uses `sfTexture_updateFromRenderWindow`, since `SF::Window::Window` is a render window. |
...
| `SF`, `SF::System::Sleep` | `SF.sleep!` and the namespace-style `SF::System::Sleep.sleep!` |
