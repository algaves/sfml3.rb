...
* `bundle exec rubocop` does not pass on the repository as a whole (67 offenses across 13 files, 49 autocorrectable, mostly `Style/FrozenStringLiteralComment` and the gemspec's 4-space indentation) — this predates the packaging work and `check.yaml` has been failing on it.
