...
### Changed
* **Minimum Ruby raised from 2.5.0 to 2.7.8**, and CI now proves it. `>= 2.5.0` had never been exercised — every workflow ran only Ruby 3.3 — and it was not actually supportable: RuboCop 1.91 requires Ruby >= 2.7.0, so the repo's own lint could not run on its declared floor. `.rubocop.yml`'s `TargetRubyVersion` moved to 2.7 to match.
* `test.yaml` now runs the suite across a Ruby matrix (2.7.8, 3.0, 3.1, 3.3, 4.0) instead of 3.3 alone, so the extension is compiled from source on every supported Ruby on each PR. 2.7 and 3.0 are the entries that matter most: no binary gem covers them, so they exercise the `ext/ports.rb` install path.
* `publish.yaml` gained a `verify-source` gate that installs the freshly built source gem on Ruby 2.7.8 and 3.0 and requires `sfml` — a release can no longer ship a source gem that fails to build on the Ruby it claims to support.
* `Gemfile` gained a `:test` group pinning `minitest` to `~> 5.0`; minitest 6 requires Ruby >= 3.2 and would otherwise have made the 2.7.8 floor untestable. The cross-compile containers now install with `BUNDLE_WITHOUT="development test"`.
...
* `bundle exec rubocop` does not pass on the repository as a whole (64 offenses across 13 files, mostly `Style/FrozenStringLiteralComment` and the gemspec's 4-space indentation) — this predates the packaging work and `check.yaml` has been failing on it.
