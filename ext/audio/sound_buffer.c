...
static VALUE SoundBuffer_from_memory(VALUE klass, VALUE rb_data) {
    ...
    return SoundBuffer_wrap(klass, sfSoundBuffer_createFromMemory(RSTRING_PTR(rb_data),
...
static void SoundBuffer_default_channel_map(unsigned int channel_count, sfSoundChannel *map) {
    static const sfSoundChannel surround[] = {
        sfSoundChannelFrontLeft,      sfSoundChannelFrontRight,     sfSoundChannelFrontCenter,
        sfSoundChannelLowFrequencyEffects, sfSoundChannelBackLeft,  sfSoundChannelBackRight,
        sfSoundChannelSideLeft,       sfSoundChannelSideRight,      sfSoundChannelTopCenter
    };
    unsigned int i;
...
/* Copies the samples into a contiguous int16 buffer: an Array is converted
   element by element, a String is memcpy'd out of (possibly unaligned) Ruby
   storage. The channel map, when given, is translated symbol by symbol. */
...
static VALUE SoundBuffer_save_to_file(VALUE self, VALUE rb_path) {
    return BOOL2RB(sfSoundBuffer_saveToFile(Get_SoundBuffer_Struct(self), StringValueCStr(rb_path)));
}
...
void Init_SoundBuffer(VALUE rb_module) {
    rb_cSoundBuffer = rb_define_class_under(rb_module, "SoundBuffer", rb_cObject);
