...
static VALUE Circle_get_inverse_transform(VALUE self) {
    return Transform_MatrixToArray(sfCircleShape_getInverseTransform(Get_Circle_Shape(self)).matrix);
}
...
static VALUE Circle_draw(VALUE self, VALUE rb_target, VALUE rb_state) {
    ...
    TARGET_DRAW(Get_Target_Struct(rb_target), sfRenderWindow_drawCircleShape, sfRenderTexture_drawCircleShape,
