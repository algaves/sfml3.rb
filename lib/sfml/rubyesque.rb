...
module SF
  ...
  #
  # `SF.sleep` gained a banged primary name so it reads like the rest of the
  ...
  class << self
    ...
    #
    # @example
    #   SF.sleep!(Time.seconds(0.25))
    def sleep!(duration)
      System.sleep(duration)
    end

    # @deprecated Use {SF.sleep!} instead.
    # @param duration [Time, Numeric] how long to sleep
    # @return [Time, Numeric] +duration+
    def sleep(duration)
      warn 'SF.sleep is deprecated; use SF.sleep!', uplevel: 1
      sleep!(duration)
    end
  end
  module System
    # `SF::System::Sleep.sleep!` mirrors the issue's namespace-style spelling and simply
    # delegates to `SF.sleep!`.
    module Sleep
      # Pauses for +duration+; an alias of {SF.sleep!}.
      #
      # @param duration [Time, Numeric] how long to sleep
      # @return [Time, Numeric] +duration+
      #
      # @example
      #   SF::System::Sleep.sleep!(Time.milliseconds(100))
  ...
  module Window
    ...
    class Window
      ...
      #     target.draw sprite
      #   end
      def render!(clear_color: SF::Graphics::Color::BLACK)
        clear!(clear_color)
  ...
  module Graphics
    ...
    class View
      ...
      def self.[](*args)
        ...
        when 2
          new.tap do |view|
            view.center = args[0].is_a?(Array) ? SF::System::Vector2[*args[0]] : args[0]
            view.size = args[1].is_a?(Array) ? SF::System::Vector2[*args[1]] : args[1]
          end
        else
    ...
    class RenderTexture
      ...
      def self.[](size, settings = nil)
        new(SF::System::Vector2[size], settings)
      end
    end
  ...
  module Window
    ...
    module Joystick
      class << self
        ...
        def has_axis?(joystick, axis)
          warn 'SF::Window::Joystick.has_axis? is deprecated; use Joystick.axis?', uplevel: 1
          axis?(joystick, axis)
        end
      end
    end
    ...
    module Keyboard
      class << self
        ...
        # @return [Boolean]
        def pressed?(key)
          warn 'SF::Window::Keyboard.pressed? is deprecated; use Keyboard.key_pressed?', uplevel: 1
          key_pressed?(key)
        end
      end
    end
    ...
    module Clipboard
      class << self
        ...
        # @return [String]
        def string
          warn 'SF::Window::Clipboard.string is deprecated; use Clipboard.content', uplevel: 1
          content
        end
