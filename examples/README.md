...
| ------------------ | ------------------------------------- | ------------------------------------------------------------------------------------- |
| Window             | Owns the OS window and render loop    | `Window.new(VideoMode.new(w, h, bpp), title)`, `frame_rate=`, `clear`, `display`, `is_open?`, `close!` |
| Events             | Input and window notifications        | `Event.new`, `poll_event!(event)`, `event.type`, `event.key`, `event.mouse_button`, `event.mouse_move`, `event.mouse_wheel_scroll` |
