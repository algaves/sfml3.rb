...
#   bundle exec ruby -Ilib examples/games/racing_car.rb
...
hud = ExampleSupport.text('', size: 16, position: [12, 10])
big = ExampleSupport.text('', size: 30, position: [80, 90])

grass = ExampleSupport.procedural_texture(32, 32) do |x, y|
...
loop do
  ...
  cars&.each_with_index do |car, index|
    car_sprite = ExampleSupport.sprite(:car, scale: 2)
