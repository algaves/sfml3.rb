...
#   bundle exec ruby -Ilib examples/games/tetris/tetris.rb
...
SPRITES = { tetro_i: [0, 1], tetro_o: [1, 1], tetro_t: [2, 1], tetro_s: [3, 1],
            tetro_z: [4, 1], tetro_j: [5, 1], tetro_l: [6, 1] }.freeze
...
clock = Clock.new
rotate_sound = sound('beep', volume: 30)
clear_sound = sound('eat', volume: 45)
over_sound = sound('explode', volume: 50)
hud = text('', size: 16, position: [8, 6])
...
def draw_cell(window, sprite_name, column, row, alpha = 255)
  sprite = sprite(sprite_name, scale: CELL / 16.0)
...
while window.open?
  ...
  window.draw(text('NEXT', size: 16, position: [sidebar_x, HUD + 8]))
  game.next_queue.compact.each_with_index do |key, index|
    ROTATIONS.fetch(key)[0].each do |x, y|
      sprite = sprite(:"tetro_#{key}", scale: (CELL - 8) / 16.0)
  ...
  window.draw(text('HOLD', size: 16, position: [sidebar_x, hold_y]))
  if game.hold_piece
    ROTATIONS.fetch(game.hold_piece)[0].each do |x, y|
      sprite = sprite(:"tetro_#{game.hold_piece}", scale: (CELL - 8) / 16.0)
  ...
  if game.state == :game_over
    ...
    window.draw(text('PAUSED -- P resumes', size: 24, position: [40, HUD + (BOARD_H / 2)]))
  end
