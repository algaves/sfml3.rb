...
# U undoes, R restarts, escape returns to the menu.
#
# Run from the repository root with a display:
#   bundle exec ruby -Ilib examples/games/chess.rb
...
status = ExampleSupport.text('', size: 18, position: [BOARD_X, 20])
help = ExampleSupport.text('click a piece, then a target   U undo   R restart   escape quits',
...
centered = lambda do |text, size, color, center_x, center_y|
  label = ExampleSupport.text(text, size: size, color: color)
...
loop do
  ...
  window.draw(ExampleSupport.text('captured by white', size: 14, position: [sidebar_x, WINDOW_H - 300]))
  window.draw(ExampleSupport.text(game.captured[:black].map { |piece| GLYPHS[piece[:type]] }.join(' '),
                                  size: 20, position: [sidebar_x, WINDOW_H - 278]))
  window.draw(ExampleSupport.text('captured by black', size: 14, position: [sidebar_x, WINDOW_H - 236]))
  window.draw(ExampleSupport.text(game.captured[:white].map { |piece| GLYPHS[piece[:type]] }.join(' '),
                                  size: 20, position: [sidebar_x, WINDOW_H - 214]))

  if pending
    window.draw(ExampleSupport.text('promote to:', size: 16, position: [sidebar_x, BOARD_Y + 8]))
