...
| `system.rb` | `Clock.measure`, `Clock#restart!`/`running?`, `SF.sleep!`/`SF::System::Sleep.sleep!`, the `Clipboard.content` pair with `has_text?`/`clear!` |
...
`SF::Window::Style` / `SF::Window::State` flag namespaces -- see [`styles.rb`](styles.rb).
