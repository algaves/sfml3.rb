...
RSpec.describe SF::Graphics::Image do
  it 'constructs from a color and reads pixels' do
    image = SF::Graphics::Image.from_color([4, 4], SF::Graphics::Color.new(10, 20, 30, 255))
  ...
  it 'sets pixels and copies' do
    image = SF::Graphics::Image.new([2, 2])
    image.set_pixel(0, 0, SF::Graphics::Color::RED)
  ...
  it 'constructs from packed pixels' do
    ...
    image = SF::Graphics::Image.from_pixels([2, 1], pixels)
    expect(image.pixel(0, 0).to_a).to eq([255, 0, 0, 255])
    expect(image.pixel(1, 0).to_a).to eq([0, 255, 0, 255])
  end

  it 'saves to memory' do
    image = SF::Graphics::Image.from_color([2, 2], SF::Graphics::Color::WHITE)
    buffer = image.save_to_memory
    expect(buffer).to be_a(SF::System::Buffer)
