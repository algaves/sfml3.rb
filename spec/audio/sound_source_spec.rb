...
RSpec.describe SFML::SoundSource do
  it 'makes SoundSource the base of the playable classes' do
    expect(SFML::Sound).to be < SFML::SoundSource
    expect(SFML::SoundStream).to be < SFML::SoundSource
    expect(SFML::Music).to be < SFML::SoundStream
    expect(SFML::Music).to be < SFML::SoundSource

    expect(SFML::SoundStream).not_to be < SFML::Sound
    expect(SFML::Music).not_to be < SFML::Sound
    expect(SFML::SoundSource).not_to be < SFML::Sound
    expect(SFML::SoundSource.superclass).to eq(Object)
  end
  ...
  it 'carries the shared playback predicates on SoundSource' do
    ...
    %i[playing? paused? stopped?].each do |name|
      expect(SFML::SoundSource.instance_methods(false)).to include(name)
    end
  end

  it 'puts the playback predicates on SoundSource' do
    %i[playing? paused? stopped?].each do |name|
      expect(SFML::SoundSource.instance_methods).to include(name)
    end
  end

  it 'shares the banged playback between every source' do
    [SFML::Sound, SFML::SoundStream, SFML::Music].each do |klass|
  ...
  it 'exposes the enum constants' do
    expect(SFML::SoundStatus::STOPPED).to eq(0)
    expect(SFML::SoundStatus::PLAYING).to eq(2)
    expect(SFML::SoundChannel::UNSPECIFIED).to eq(0)
    expect(SFML::SoundChannel::FRONT_LEFT).to be_a(Integer)
  end

  it 'constructs and compares sound source cones' do
    cone = SFML::SoundSourceCone.new(10, 20, 0.5)
    ...
    expect(SFML::SoundSourceCone.new(10, 20, 0.5)).to eq(cone)
  end
end
