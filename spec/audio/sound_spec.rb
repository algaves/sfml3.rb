...
RSpec.describe SFML::Sound do
  it 'round-trips sound properties' do
    buffer = SFML::SoundBuffer.from_samples([0, 0, 0, 0], 1, 44_100)
    ...
    sound.cone = SFML::SoundSourceCone.new(90, 180, 0.1)
  ...
  it 'assigns and clears an effect processor' do
    buffer = SFML::SoundBuffer.from_samples([0, 0], 1, 44_100)
    sound = SFML::Sound.new(buffer)

    sound.effect_processor = proc { |frames, _channels| frames }
    sound.effect_processor = nil
    expect(sound).to be_a(SFML::Sound)
  end
end
